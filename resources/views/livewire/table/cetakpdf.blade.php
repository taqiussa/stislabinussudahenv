{{-- Table Data --}}
<div>
    <x-data-table-tagihan :data="$data" :model="$cetakpdfs">
        <x-slot name="head">
            <tr>
        ...
        <x-slot name="body">
            @foreach ($cetakpdfs as $key => $siswa)
                <tr x-data="window.__controller.dataTableController({{ $siswa->id }})">
                    ...
                    <td><a role="button" target="_blank" href="{{ route('savepdf',['id' => $siswa->id,'uas' => $uas, 'pas' => $pas, 'pat' => $pat]) }}" class="px-4 py-2 font-bold text-white bg-blue-500 rounded hover:bg-blue-700">Download</a>
                    </td>
                </tr>
            @endforeach
        </x-slot>
    </x-data-table-tagihan>
</div>